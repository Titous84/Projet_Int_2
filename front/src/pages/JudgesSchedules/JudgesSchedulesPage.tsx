...
export default class JudgesSchedulesPage extends React.Component<{}, JudgesSchedulesPageState> {
  ...
  */
  async loadTimeSlots() {
    const response = await JugeStandService.GetAllTimeSlots();
    if (response.error) {
  ...
  */
  async loadEvaluation() {
    const response = await JugeStandService.GetAllEvaluations();
  ...
  */
  onChangeHour = (newValue: Date, hourNumber: number) => {
    const updatedHour = [...this.state.hours];
                    ...
                    this.state.hours.map((element, index) => {
                      return (
                        <TableCell
                          ...
                          align='center'
                        >
                          {("0" + element.time.getHours()).slice(-2)}:{("0" + element.time.getMinutes()).slice(-2)}
                        </TableCell>
                      );
                    })
                  }
