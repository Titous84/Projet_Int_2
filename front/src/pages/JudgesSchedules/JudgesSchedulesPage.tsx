...
export default class JudgesSchedulesPage extends React.Component<
  ...
  JudgesSchedulesPageState
 {
  ...
  async loadTimeSlots() {
    ...
    if (response.error) {
      ShowToast(
        'Une erreur est survenue lors du chargement des plages horaires.',
        5000,
        'error',
        'top-center',
        false,
  ...
  async AddSurvey(
    ...
    const response = await JugeStandService.PushStandSurvey(
      jugeId,
      standId,
      surveyId,
      hour,
  ...
  verifyIfTeamIsAssignedMoreThan3Times(
    ...
    const countAssignation = standEvalArray.filter(
      (assignation) => assignation.stand_id === stand_id,
    ).length;
  ...
  callbackStandEval = (evaluation: IAssignation) => {
    if (
      this.verifyIfTeamIsAssignedMoreThan3Times(
        this.state.standsEval,
        evaluation.stand_id,
      )
    ) {
      ShowToast(
        "L'équipe a été assignée plus de 3 fois",
        5000,
        'warning',
        'top-center',
        false,
  ...
  handleChangeAssignation = (evaluation: IAssignation) => {
    const existingAssignationIndex = this.state.standsEval.findIndex(
      (assignation) => assignation.id === evaluation.id,
    );
  ...
  handleDeleteAssignation = (id: number) => {
    const updatedStandsEval = this.state.standsEval.filter(
      (assignation) => assignation.id !== id,
    );
  ...
  handleAddTimeSlot = async (heureDepart: Date, interval: number) => {
    ...
    if (this.state.hours.length <= 0) {
      ...
      const prochainHeure = new Date(
        dernierHeure.getTime() + interval * 60 * 1000,
      );
      nouveauSlot = {
        id: 0,
        time: prochainHeure,
    ...
    try {
      ...
      ShowToast(
        "Une erreur est survenue lors de l'ajout de la plage horaire.",
        5000,
        'error',
        'top-center',
        false,
